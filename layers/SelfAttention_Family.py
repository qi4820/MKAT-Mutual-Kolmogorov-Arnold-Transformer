...
class MKAT_TwoStageAttentionLayer(nn.Module):
    '''
    The Two Stage Attention (TSA) Layer
    input/output shape: [batch_size, Data_dim(D), Seg_num(L), d_model]
    '''
    ...
    def __init__(self, configs,
                 seg_num, factor, d_model, n_heads, d_ff=None, dropout=0.1,activation="relu", num_groups=8, modular_kan = None):
        super(MKAT_TwoStageAttentionLayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.time_attention = AttentionLayer(FullAttention(False, configs.factor, attention_dropout=configs.dropout,
                                                           output_attention=False), d_model, n_heads)
        self.dim_sender = AttentionLayer(FullAttention(False, configs.factor, attention_dropout=configs.dropout,
        ...
        if self.use_group_kan :
            ...
            self.group_kan_1 = KAT_Group(num_groups=num_groups, mode=mode, device=device)
            self.group_kan_2 = KAT_Group(num_groups=num_groups, mode=mode, device=device)
            # self.activation = None  # 不再使用 F.relu/F.gelu
            self.linear1_1 = nn.Linear(d_model, d_ff)
            self.linear1_2 = nn.Linear(d_ff, d_model)
            self.linear2_1 = nn.Linear(d_model, d_ff)
            self.linear2_2 = nn.Linear(d_ff, d_model)
            # print("using kan")
        else:
            self.MLP1 = nn.Sequential(nn.Linear(d_model, d_ff),
                                  nn.GELU(),
        ...
        # 在中间去加入的一个KAN
        self.modular_kan = modular_kan

    def forward(self, x, attn_mask=None, tau=None, delta=None,
    global_shared_denominator = None):
        # Cross Time Stage: Directly apply MSA to each dimension
        batch = x.shape[0]
        # ========== Cross Time Stage ==========
        time_in = rearrange(x, 'b ts_d seg_num d_model -> (b ts_d) seg_num d_model')
        ...
        # 然后他的这个地方他是先把他去残差一下然后再去送入Modular_kan的，然后这样子的做法也是与我们之前做的MKAT是相同的，先去送到modular里面再去做group
        dim_in = time_in + self.dropout(time_enc)
        dim_in = self.norm1(dim_in)

        # 👉 插入点 A: Modular KAN in d_model space (after time attention)
        # print(self.modular_kan,global_shared_denominator)
        if self.modular_kan is not None and global_shared_denominator is not None:
            # reshape to [B', seg_num, d_model] -> apply modular KAN
            # print("encoder里面使用了modular_kan")
            dim_in = self.modular_kan(dim_in, global_shared_denominator)

        # FFN / Group KAN in d_ffn space
        if self.use_group_kan:
            # print("使用了第一个group_kan")
            y = self.linear1_1(dim_in)              # [B', seg_num, d_ff]
            # y = y.transpose(-1, -2)                 # [B', d_ff, seg_num]
            # y = y.transpose(-1, -2)                 # back to [B', seg_num, d_ff] for KAT_Group
            y = self.group_kan_1(y)                 # Apply Group KAN
            y = self.linear1_2(y)                   # [B', seg_num, d_model]
        else:
            y = self.MLP1(dim_in)


        dim_in = dim_in + self.dropout(y)
        dim_in = self.norm2(dim_in)
        ...
        # # 👉 插入点 B: Modular KAN in d_model space (after dim attention)
        # if self.modular_kan is not None and global_shared_denominator is not None:
        #     dim_enc = self.modular_kan(dim_enc, global_shared_denominator)
        # 现在说这个地方不太适合去加了

        # FFN / Group KAN in d_ffn space
        if self.use_group_kan:
            # print("使用了第二个group_kan")
            y = self.linear2_1(dim_enc)             # [B'', ts_d, d_ff]
            # y = y.transpose(-1, -2)                 # [B'', d_ff, ts_d]
            # y = y.transpose(-1, -2)                 # back to [B'', ts_d, d_ff]
            y = self.group_kan_2(y)
            y = self.linear2_2(y)
        else:
            y = self.MLP2(dim_enc)

        dim_enc = dim_enc + self.dropout(y)
        dim_enc = self.norm4(dim_enc)
