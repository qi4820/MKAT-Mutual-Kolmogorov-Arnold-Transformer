...
class MKAT_PastDecomposableMixing(nn.Module):
    def __init__(self, configs,activation="relu", num_groups=8, encoder_kan = None):
        super(MKAT_PastDecomposableMixing, self).__init__()
        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.down_sampling_window = configs.down_sampling_window
        ...
        print("self.channel_independence:",self.channel_independence)
        if self.use_kan:
            # if not HAS_KAN:
            ...
            self.kan1 = KAT_Group(num_groups=num_groups, mode=mode, device=device)
            self.kan2 = KAT_Group(num_groups=num_groups, mode=mode, device=device)
    ...
    def forward(self, x_list,global_shared_denominator = None):
        length_list = []
        for x in x_list:
            _, T, _ = x.size()
            length_list.append(T)
        ...
        for x in x_list:
            ...
            if not self.channel_independence:
                if self.use_kan:
                    season = self.kan1(season)
                    trend = self.kan1(trend)
                else:
                    season = self.cross_layer(season)
                    trend = self.cross_layer(trend)
        ...
        for ori, out_season, out_trend, length in zip(x_list, out_season_list, out_trend_list,
                                                      length_list):
            out = out_season + out_trend
            # 这个地方他就是一个新的d_model里面的一个序列了。
            if self.encoder_kan is not None and global_shared_denominator is not None:
                # print("现在在伪encoder里面去使用了kan了")
                out = self.encoder_kan(out, global_shared_denominator)
            # 然后下面的这个东西就是他的ffn中的一个路线，我们这个地方也是去做一个像他这个的残差的设计
            if self.channel_independence:
                if self.use_kan:
                    # print("现在已经使用group_kan去代替了mlp")
                    out = ori + self.kan2(out) 
                else:
                    out = ori + self.out_cross_layer(out) 
            out_list.append(out[:, :length, :])
        return out_list
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        print(configs.activation)
        self.pdm_blocks = nn.ModuleList([MKAT_PastDecomposableMixing(configs,activation=configs.activation,num_groups = configs.num_groups,encoder_kan = self.encoder_kan,)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # 这个是第一个插入在embeding后面的那个MKAT_modular_kan
        if self.use_shared_kan:
            enc_out = self.embed_kan(enc_out, self.shared_denominator)  # ← use shared D
            # print("现在是在embeding后面去使用了kan了")

        # Past Decomposable Mixing as encoder for past
        for i in range(self.layer):
            enc_out_list = self.pdm_blocks[i](enc_out_list, self.shared_denominator)
