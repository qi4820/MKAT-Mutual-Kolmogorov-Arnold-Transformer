...
def Rational_CUDA_A_1DGroup(x, weight_numerator, weight_denominator, group):
    ...
    x: 输入张量，shape=(B, L, D)
    weight_numerator: 分子参数，shape=(num_groups, degree_num)
    weight_denominator: 分母参数，shape=(num_groups, degree_den)
    group: int，分组数（与 KAT_Group_Torch.num_groups 一致）
...
class KAT_Group_Torch(nn.Module):
    def __init__(self, num_groups=8, mode="gelu"):
        ...
        Args:
            num_groups (int): Number of groups to divide the input for separate processing.  num_groups: int，通道分组数。决定有多少套分子/分母参数，输入的最后一维（channels）必须能被 num_groups 整除。
            init_mode (str): Initialization mode which determines the preset weights from JSON file.  mode: str，激活函数类型（如"gelu"、"swish"、"identity"），决定从 init.json 加载哪种初始化系
        """
    ...
    def forward(self, input):
        ...
        Returns:
            ...
            B：batch size
            L：序列长度（或空间位置）
            D：特征维度（channels），必须满足 D % num_groups == 0  que:这个地方他为什么要求D%num_groups == 0?:
        """
